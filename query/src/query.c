/* ========================================================================== */
/* File: query.c - Tiny Search Engine query engine
 *
 * Author: Chua Zheng Fu Edrei			
 * Date: 14 August 2015
 *
 * Input: ./query [INDEX FILE] [HTML DIR]
 *
...
int main(int argc, char* argv[]){
	...
	while(fgets(text,MAXLEN, stdin)){
		...
		if(flag){
			...
			printf("\nPrinting the top %d results or less:\n",MAXRESULT);
...
int StringToWord(char wordarray[][MAXLEN], char text[MAXLEN]){
	...
	char* word = NULL;
	i = pos = 0;
    while ((pos = GetNextWord(text, pos, &word)) > 0 && i<MAXLEN-1){	// get words from the text
